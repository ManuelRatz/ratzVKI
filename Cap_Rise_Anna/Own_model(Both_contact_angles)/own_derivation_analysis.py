...
for i in range(0, 3):
    ...
    h_cl = (data_cl_l + data_cl_r) / 2
    ca = (data_lca + data_rca) / 2
    # shift the data
    shift_idx = np.argmax(h_cl > 0)
    h_cl = h_cl[shift_idx:]
    ...
    h_cl = savgol_filter(h_cl, 55, 3, axis = 0)
    ...
    t = np.arange(0, len(h_cl)/500, 0.002)
    #set up the saving folder for the images
    FOL_OUT = '%d_pa_images' %Pressure[i] + os.sep
    
    # plot the raw contact angles in comparison with the averaged one
    
    fig, ax = plt.subplots(figsize = (8, 5))
    plt.plot(t, np.cos(data_rca), label = 'cos$(\Theta_L)$')
    plt.plot(t, np.cos(data_lca), label = 'cos$(\Theta_R)$')
    ax.set_xlabel('$t$[s]')
    ax.set_ylabel('cos($\Theta$)')
    ax.set_xlim([0, 4])
    plt.legend(loc = 'lower right')
    plt.title('Comparison of the cosinus of the left and right contact angle (%d Pa)' %Pressure[i])
    ...
    plt.plot(t, h_cl*1000, label = 'Experimental data')
    ax.set_xlabel('$t$[s]')
    ax.set_ylabel('$h$[mm]')
    ax.set_xlim([0, 4])
    ax.set_ylim([h_cl[0]*1000, np.amax(sol_filter[:,1]*1000)+2])
    plt.legend(loc = 'lower right')
    ...
    ax.set_ylabel('$h$[mm]')
    ax.set_xlim([0, 4])
    ax.set_ylim([h_cl[0]*1000, np.amax(sol_step[:,1]*1000)+2])
    plt.legend(loc = 'lower right')
    ...
    plt.plot(t, h_cl*700, label = 'Contact line height')
    ...
    plt.plot(t, acc_filter[:, 4], label = 'Inertia')
    ...
    plt.plot(t, acc_step[:, 4], label = 'Inertia')
    ...
    plt.plot(t, acc_step_adv[:, 4], label = 'Inertia')
