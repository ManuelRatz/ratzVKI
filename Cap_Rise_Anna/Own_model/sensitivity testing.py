...
# Define the ODE functions, one is used for calculating the left contact angle
# the other for the right contact angle
def ode_lca(X, t):
    U, Y = X
    dudt = (Y)**(-1)*(-g*Y - 12*(l+delta)*mu*U*Y/(rhoL*l*delta**2) + pres(t)/rhoL\
                      + 2.0*(l+delta)*sigma*np.cos(lca(t))/(rhoL*l*delta) - U*abs(U))
    dydt = U
    return [dudt, dydt]
...
def ode_rca(X, t):
    U, Y = X
    dudt = (Y)**(-1)*(-g*Y - 12*(l+delta)*mu*U*Y/(rhoL*l*delta**2) + pres(t)/rhoL\
                      + 2.0*(l+delta)*sigma*np.cos(rca(t))/(rhoL*l*delta) - U*abs(U))
    dydt = U
    return [dudt, dydt]
...
for i in range(0, 3):
    ...
    # shift the data
    shift_idx = np.argmax(rca_load > 0)
    rca = rca_load[shift_idx:]
    lca = lca_load[shift_idx:]
    h_avg_load = h_avg_load[shift_idx:]
    pressure_load= pressure_load[shift_idx:len(rca)+shift_idx]
    
    # smooth the data 
    lca_smoothed = lca#savgol_filter(rca, 9, 3, axis = 0)
    rca_smoothed = rca#savgol_filter(lca, 9, 3, axis = 0)
    ...
    t = np.arange(0, len(rca)/500, 0.002)
    ...
    lca_inter = interpolate.splrep(t, lca_smoothed)
    def lca(t):
        return interpolate.splev(t, lca_inter)
    rca_inter = interpolate.splrep(t, rca_smoothed)
    def rca(t):
        return interpolate.splev(t, rca_inter)
    ...
    solution0 = odeint(ode_rca, X0, t)
    solution1 = odeint(ode_rca, X1, t)
    solution2 = odeint(ode_rca, X2, t)
    ...
    sol_offset = odeint(ode_lca, X_off, t)
    ...
    h_avg_heavy = savgol_filter(h_avg_load, 55, 1, axis = 0)
    ...
    solution_vel_0 = odeint(ode_rca, X_0, t)
    solution_vel_1 = odeint(ode_rca, X_1, t)
    solution_vel_2 = odeint(ode_rca, X_2, t)
    ...
    """
    Now we compare the left contact angle height compared to the right contact
    angle height. For that we use two different functions (as defined above) 
    and calculate eacht one separetly
    """
    # set up the solutions using the unsmoothed velocity with no offset
    solution_rca = odeint(ode_rca, X0, t)
    solution_lca = odeint(ode_lca, X0, t)
    ...
    ax.set_xlim([0, 4])
    # ax.set_ylim([71, 118])
    plt.plot(t, solution_rca[:,1]*1000, c = 'k', label = 'Right contact angle')
    plt.plot(t, solution_lca[:,1]*1000, c = 'b', label = 'Left contact angle')
    # plt.plot(t, h_smoothed*1000, c = 'r', label = 'Experimental data')
    plt.legend(loc = 'lower right')
    # plt.title('Comparison of different smoothing options for the velocity (%d Pa)' %pressure_case[i])
    plt.savefig(FOL_OUT + 'Comparison_rca_lca_%d.png' %pressure_case[i])
