...
# threshold for the kernel filtering7
...
Fol_Data, Pressure, Run, H_Final, Frame0, Crop_Index, Speed = imp.get_parameters(Test_Case, Case, Fluid)
N_T = Frame0+Img_Amount-1
Pix2mm = Width/(Crop_Index[1]-Crop_Index[0]) 
# name of the output folder
Fol_Out = ppf.create_folder('Crop_Check')
Fol_Images_Detected = ppf.create_folder(Fol_Data + os.sep + 'Images_Detected_cosh')
...
GIFNAME = 'Detected_interface_cosh.gif' # name of the gif
images2 = []
GIFNAME2 = 'Detected_interface_cosh_glob.gif' # name of the gif
P0 = [1,1,1,1]
...
for i in range(0,Img_Amount):
    ...
    # Y_c = Y_c/np.max(Y_c)
    if Idx >=100:
        Y_fit = imp.fitting_cosh2(X_c, Y_c) # fit to a cosine  
    else:
        Y_fit, P0 = imp.fitting_cosh(X_c, Y_c, P0) # fit to a cosine    
    ...
    if (Idx % 3) == 0:
        ...
        ax.scatter(i_x, i_y, marker='x', s=(100./fig.dpi)**2, color = 'red') # plot the detected gradient onto the image
        ax.plot((X)/(Pix2mm), Y_fit, 'yellow', linewidth=0.5) # plot the interface fit
        ax.invert_yaxis()
        ax.set_aspect('equal')
        ax.axis('off') # disable the showing of the axis
        ax.set_ylim(mu_s[500]-20, mu_s[500]+65)
        NAME_OUT = Fol_Images_Detected + os.sep + 'Image_%05d.png' %Idx
        fig.tight_layout()
        ...
        # ax.imshow(np.flip(img, axis = 0), cmap=plt.cm.gray) # show the image in greyscale
        # ax.plot((X)/(Pix2mm), Y_fit, 'r-', linewidth=0.5) # plot the interface fit
        # ax.invert_yaxis()
        # ax.set_aspect('equal')
        # ax.axis('off') # disable the showing of the axis
        ...
        # images2.append(imageio.imread(NAME_OUT))
imageio.mimsave(GIFNAME, images, duration = 0.10)
# imageio.mimsave(GIFNAME2, images2, duration = 0.10)
