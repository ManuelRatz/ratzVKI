...
def generate_filename(folder, number, pair, pic_format):
    ...
    :return:
    """
    return folder + os.sep + 'R_h1_f1000_1_p14.6dbpi5g7.%06d' % (number + 1) + '.' + pic_format
...
Num = 260  # This is the number of the image
Name=generate_filename(FOL_IN, Num, 'a', 'tif')
...
def process_image_for_matrix_D(FOL_IN, iter, pair, X1, X2, Y1, Y2):
    ...
    Im = mpimg.imread(name)  # Read the image as uint8 using mpimg
