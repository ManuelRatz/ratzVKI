...
# for i in range(1,5):
    ...
    H = ppf.load_h(Fol_In)
    # calculate the first and last image index
    Idx1 = Periods[i,1]
    ...
    # get the maximum velocity and acceleration
    ...
    # different scales for the highpass filtered velocity fields
    ...
    for j in range(0, N_T):
        # calculate the loading indices
        Load_Idx = Stp_T*j+0 # index for the velocity profiles
        Image_Idx = Load_Idx + Frame0 # index for s2n and the images
        # load s2n ratio and the img
        s2n = ppf.load_s2n(Fol_In, Image_Idx)
        img = ppf.load_raw_image(Fol_Raw, Image_Idx)
        # calculate the first valid row of the run
        valid_row = ppf.first_valid_row(u_tensor[Load_Idx,:,:])
        
        # extract the current timestep from the 3d tensors, already cropping nans
        ...
        # unpad the velocity profiles (for the qfield)
        x = x_pad[:,1:-1]
        y = y_pad[:,1:-1]
        u = u_pad[:,1:-1]
        v = v_pad[:,1:-1]
