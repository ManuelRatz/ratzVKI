...
for i in range(0,N_T):
    ...
    # plot the contour and the quiver
    if PLOT_QUIV == True:
        ...
        ax.set_ylim(0,1271)
        ax.set_yticks(np.arange(0,1300,220)) # set custom y ticks (not automatized)
        ax.set_yticklabels((0,4,8,12,16,20), fontsize=15) # set custom labels (not automatized)
        ax.set_xlim(0,5)
        ax.set_xticks(np.linspace(0,275,6)) # set custom x ticks (not automatized)
        ax.set_xticklabels(np.arange(0,6,1), fontsize=15) # set custom y ticks (not automatized)
        ax.set_xlabel('$x$[mm]', fontsize=20)
        ax.set_ylabel('$y$[mm]', fontsize=20)
    ...
    if PLOT_PROF == True:
        ...
        fig, ax = plt.subplots(figsize=(8,5))
        y_IND = np.array([int(len(x)*0.25)-1,int(len(x)*0.5)-1,int(len(x)*0.75)-1]) # initialize array with values every 25% of the ROI
