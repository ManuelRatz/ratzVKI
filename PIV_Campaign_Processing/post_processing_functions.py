...
def load_and_smooth(Fol_Sol, order = 15):
    def smooth_frequency(profiles):
        ...
        def extract_profile(time_column):
            ...
            if id2 < 50:
                ...
                id2_second_attempt = np.argmax(time_column[id1+id2+2:] > 0.8*Fill_Dum)
        ...
        for i in range(0, profiles.shape[0]):
            ...
            for j in range(0, profiles.shape[1]):
                ...
                if np.mean(prof_hor) > 0.8*Fill_Dum:
                    ...
                    smoothed_pad, DUMMY, DUMMY, DUMMY = smoothn(padded_profile, s = 0.125, isrobust = True)
    ...
    for i in range(0, N_T):
        ...
        x, y, u, v, ratio, valid, invalid = filter_invalid(x, y, u, v, ratio, mask, valid_thresh = 0.5) # filter invalid rows
    ...
    inv = smoothed_tot > 0.8*Fill_Dum
    smoothed_tot[inv] = np.nan    
    
    print('Smoothing along the horizontal axis with cosine transformation')
    smoothed_tot_cos = smooth_horizontal_sin(smoothed_vert_freq, x[0,:], Width)
    # filter the values filled with the dummy
    inv = smoothed_tot_cos > 0.8*Fill_Dum
    smoothed_tot_cos[inv] = np.nan

    return tensor_x, tensor_y, profiles_u, profiles_v, smoothed_tot_cos, smoothed_tot
...
def calc_qfield(x, y, u, v):
    u_smo, dum, dum, dum = smoothn(u, s = 5)
    v_smo, dum, dum, dum = smoothn(v, s = 5)
