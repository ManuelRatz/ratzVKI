...
R = 15
# do the POD
Phi_v, Sigma_v, Psi_v = POD(V_POD_load, R)
Phi_u, Sigma_u, Psi_u = POD(U_POD_load, R)
...
Psi_v_interp = np.zeros((t_2.shape[0], R))
Psi_u_interp = np.zeros((t_2.shape[0], R))
...
Phi_v_interp = np.zeros((y_new.shape[0]*x_new.shape[0], R))
Phi_u_interp = np.zeros((y_new.shape[0]*x_new.shape[0], R))
...
u_old_hp = u_old-u_old.mean()
v_old_hp = v_old-v_old.mean()
...
u_new_hp = u_new-u_new.mean()
v_new_hp = v_new-v_new.mean()

# create the figure
Stp=1
fig, ax = plt.subplots(figsize = (8,5))
# ax.quiver(X_old, Y_old, , )
