...
#offset = 0 * np.pi / 180
#smooth all the data
pres_filter = savgol_filter(pressure, 55, 3, axis =0)
...
#==============================================================================
# For now the calculation is only done for the left contact angle
#==============================================================================


h0 = 0.074 + disp_cl_filter[0]/1000*1.1
dh0 = (disp_cl_l_filter[1] - disp_cl_l_filter[0])/(0.002*1000)


Xzero = np.array([dh0, h0])
def deriv(X,t):
    U, Y  = X 
    dudt = (Y+37./36*R)**(-1)*(-(8*mu/(rhoL*((R*C1)**2)))*U*(Y+0.25*R)-g*Y+ (pres(t)-Patm)/rhoL +(2*sigma/(rhoL*R*C1/2))*np.cos(ca_func(t))-7./6*(U)*abs(U))
    dydt = U
    return [dudt, dydt]

solvS = integrate.odeint(deriv, Xzero, t)

pressure_term = (pressure - Patm) / rhoL * (solvS[:,1] + 37. * R / 36)
surface_tension_term = 2 * sigma * np.cos(ca_filter) / (rhoL * R * C1*2) * (solvS[:,1] + 37. * R / 36)
gravity_term = -g * solvS[:,1] * (solvS[:,1] + 37. * R / 36)
visc_term = -8*mu*solvS[:,1]*solvS[:,0] / (rhoL * R**2 * C1**2) * (solvS[:,1] + 37. * R / 36)
corr_term_1 = -2*mu*R*solvS[:,0] / (rhoL * R**2 * C1**2) * (solvS[:,1] + 37. * R / 36)
corr_term_2 = -7. * solvS[:,0]*abs(solvS[:,0])/6 * (solvS[:,1] + 37. * R / 36)
...
plt.legend()
"""
plt.plot(t, (pressure_term + gravity_term))
plt.plot(t, total)
...
plt.plot(t, (total*1000)+100, label = 'Acceleration')
